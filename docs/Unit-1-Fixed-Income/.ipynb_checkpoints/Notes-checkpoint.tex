...
Lattice models are mathematical models used to simulate the behavior of stochastic systems over time. 
...
of states (called nodes or vertices) connected by branches. 
...
model and then extend to multi-period models. 
In subsequent modules, we'll explore how lattice models can simulate equity prices, derivatives prices, and other financial variables.
...
\begin{definition}[Bernoulli distribution]\label{def:bernoulli-distribution}
...
p_{X}(x) = \begin{cases}
	...
	\end{cases}
\end{equation}
where $0<p<1$ is called the Bernoulli parameter. The expectation of a Bernoulli random variable is given by:
\begin{equation}
...
We could develop a model to calculate these values based on some understanding or belief about the underlying financial dynamics, which is what we'll do in the case of derivatives. Alternatively, we could estimate these values from historical data, which we'll do here.
...
There are two approaches for computing the tuple $(u,d,p)$: real-world probabilities, which can be estimated from historical data, and risk-neutral probabilities, 
...
Here, we'll focus on estimating the real-world probabilities $(u,d,p)$ from historical data, particularly interest rate data reported and published daily.
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
		...
		and $\mathcal{D}$ denotes the data set of all T-bills. There are $N$ observations in the dataset.
        \Require The time interval $\Delta{t}$ between $(t-1)\rightarrow{t}$ (units: years) and a list of maturities $\mathcal{L} = \left\{i\right\}_{i=1}^{M}$ for the T-bills in $\mathcal{D}$.
		There are $M$ maturities in the dataset.
		\Require All T-bills have the same number of observations $N$.
     
        \Statex
		\Procedure{log growth rate}{$\mathcal{D}$, $\mathcal{L}$, $\Delta{t}$}
		\State{$N\leftarrow\text{length}(\mathcal{D})$}\Comment{Number of observations for $i\in\mathcal{L}$}
        \For{$i\in\mathcal{L}$}
            ...
            \For{$t=2\rightarrow{N}$}
                \State{$r_{i,t}, r_{i,t-1} \gets \mathcal{D}_{i}[t], \mathcal{D}_{i}[t-1]$}\Comment{Select the rate $i$ at times $t$ and $t-1$}
                \State{$\mu^{(i)}_{t,t-1} \gets \left(1/\Delta{t}\right)\cdot\ln\left(r_{i,t}/{r_{i,t-1}}\right)$}
            \EndFor
        \EndFor
        \Statex
        \Return{$\mu^{(1)},\dots,\mu^{(M)}$}\Comment{Return the logarithmic growth rate array for each T-bill $i\in\mathcal{L}$}
		\EndProcedure
    \end{algorithmic}
\end{algorithm}
...
\begin{concept}[Estimating real-world probabilities]\label{concept:real-world-interest-rate-probabilities}
We can estimate the real-world probability $p$ by counting the number of times $\mu_{j,j-1}\leq{0}$ and 
dividing by the total number of observations. Similarly, we can estimate values for the \texttt{up} and \texttt{down} factors $u$ and $d$ 
by computing the average of $exp(\mu_{j,j-1}\cdot{\Delta{t}})$ when the rate increases or decreases, respectively.
\end{concept}
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		...
		\Require The time interval $\Delta{t}$ between $(t-1)\rightarrow{t}$ (units: years)
		\Require \texttt{mean} function, \texttt{find} function, \texttt{length} function, and \texttt{push} function.

		\Statex
		\Procedure{real world probability}{$\mu$, $\Delta{t}$}
		% \State
		...
		% \State \textit{Compute $u$:}
   	 	\State{$i_{+}\leftarrow \text{find}(\mu\geq{0})$}\Comment{Find the indices of all \texttt{non-negative} growth rates}
    	\For{$i\in {i_{+}}$}
			\State{$\text{push}(\text{up}, \exp(\mu[i]\cdot{\Delta{t}})))$}\Comment{Push the \texttt{non-negative} values into $\text{up}$-array}
    	\EndFor
    	...
    	\For{$i\in {i_{-}}$}
			\State{$\text{push}(\text{down}, \exp(\mu[i]\cdot{\Delta{t}}))$}\Comment{Push the \texttt{negative} values into $\text{down}$-array}
    	\EndFor
		\State{$d\leftarrow\text{\texttt{mean}}(\texttt{down})$}\Comment{mean is our estimate of the \texttt{down} factor $d$}

		\Statex
		\State{$N_{+} \leftarrow\text{length}(i_{+})$}\Comment{Number of \texttt{non-negative} growth rates}
		\State{$p\leftarrow N_{+}/N$}\Comment{Estimate of the probability of an up move $p$}
		\Statex

		\Return{$u$, $d$ and $p$}\Comment{Return the estimated \texttt{up}, \texttt{down} and probability $p$ factors}
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
Thus, we use the same $(u,d,p)$ tuple to model the interest rate dynamics (or other quantities) at each time step.
\begin{figure}[h]
    \centering
	...
	Then, from the \texttt{up} state, we can move to the \texttt{up} state with probability $p$ or the \texttt{down} state with probability $(1-p)$, and so on. 
	}\label{fig:example-multistep-binomial-lattice-schematic}
\end{figure}
% \begin{definition}[Binomial Distribution]\label{def:binomial-distribution}
...
\subsubsection{Multiple-period binomial lattice analytical solution}
The simplifying assumption of a constant $(u,d,p)$ tuple allows us to develop an analytical solution for the interest rate at time $t$ in the future.
Consider a binomial lattice model with $n$ time-steps. 
During each time step, the interest rate can either go \texttt{up} by a factor of $u$ or \texttt{down} by a factor of $d$.
...
As the number of time steps increases, we have a series of Bernoulli trials, which leads to a binomial distribution (Defn: \ref{defn-binomial-distribution-rate}):
\begin{definition}[Binomial Lattice Analytical Solution]\label{defn-binomial-distribution-rate}
	Let $r_{\circ}$ denote the current rate at t = \texttt{0}, $u$ and $d$ denote the \texttt{up} and \texttt{down} factors, and $p$ denote the probability of going \texttt{up}.
	...
	\end{equation*}
	Further, the probability that rate takes on a particular value at time $t$ is given by:
	\begin{equation*}
